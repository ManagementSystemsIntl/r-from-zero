...
```{r, echo=FALSE}
# install packages
base_packages <- install.packages(c("dplyr","readxl", "tidyr", "purrr", "knitr", "rmarkdown", "readr", "sjmisc", "sjPlot"),repos = "http://cran.us.r-project.org")
...
`install.packages(c("purrr", "readxl", "sjmisc", "sjPlot"))`
...
For the breakouts, each group will be given a set of questions from a fake survey and corresponding data. Participants should use the common functions above to find as many errors in the dataset as you can!
